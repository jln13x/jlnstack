...
```ts
import { createMemoryAdapter } from "@jlnstack/notifications";
...
```ts
import type { NotificationAdapter, NotificationTypesConstraint } from "@jlnstack/notifications";
...
```ts
// Future API
import { createDrizzleAdapter } from "@jlnstack/notifications/drizzle";
